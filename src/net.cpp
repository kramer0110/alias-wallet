...
static void run_tor() {
    ...
    if (fs::exists(torrc_defaults_file) && fs::is_regular_file(torrc_defaults_file))
    {
        ...
        {
            int check_rev = 0;
            fscanf(file, "# torrc-defaults for Spectrecoin rev. %d\n", &check_rev);
    ...
    if (must_override || !fs::exists(torrc_defaults_file) || !fs::is_regular_file(torrc_defaults_file))
    {
        ...
        if (file)
        {
            fprintf(file, "# torrc-defaults for Spectrecoin rev. %d\n", TORRC_DEFAULTS_REV);
            fprintf(file, "#\n");
            fprintf(file, "# DO NOT EDIT THIS FILE\n");
            fprintf(file, "#\n");
            fprintf(file, "# This file is distributed with Spectrecoin within the tor folder and SHOULD NOT be modified\n");
            fprintf(file, "# (it may be overwritten during the next Spectrecoin update).\n");
            fprintf(file, "# To customize your Tor configuration, shut down Spectrecoin and edit the torrc file.\n");
            fprintf(file, "#\n");
